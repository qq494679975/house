...
function tipAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		try{	//更新最新的小贴士设置
			...
			showTip.isShow=("true"==json.isShow);
			showTip.period=(json.period||10);
			showTip.showEmail=("true"==json.showEmail);
			showTip.showNotice=("true"==json.showNotice);
			if(showTip.isShow){
				//如果设置的小贴士为显示
				if(!showTip._closeTipTimer){
					//如果原来定时获取小贴士没有开启
					//开启获取小贴士
					showTip.getTip();
				}
				...
				//如果设置不显示小贴士
				showTip.cancelCurrent();//取消之前可能开启的定时获取小贴士
			}
		}catch(exception){
